...
class Customer(User):
    ...
    def rent_car(self, car_id, start_date, end_date):
        ...
        if not car or not car.available:
            print(f"Car with ID {car_id} is not available.")
            return False
        ...
        try:
            ...
            total_cost = days * car.rental_price
            if self.balance < total_cost:
                print(f"Insufficient balance. Needed: ${total_cost:.2f}")
                return False
            ...
            RentalManager.add_rental(self.username, rental)
            self._update_customer_data()
            print(f"Car rented successfully for ${total_cost:.2f}")
            return True
        except ValueError as e:
    ...
    def return_car(self, car_id, return_date):
        from models.rental import RentalManager
        rental = next((r for r in self.rentals if r["car_id"] == car_id and r["status"] == "active"), None)
        if not rental:
            print(f"No active rental found for Car ID {car_id}.")
            return False
        ...
        try:
            ...
            if return_dt > end_date:
                days_late = (return_dt - end_date).days
                car = Car.get_car_by_id(car_id)
                fine = days_late * (car.rental_price * 1.5)
                self.balance -= fine
    ...
    def view_rental_history(self):
        ...
        for rental in self.rentals:
            car = Car.get_car_by_id(rental["car_id"])
            print(f"\nCar: {car.brand} {car.model} (ID: {car.car_id})")
            print(f"Period: {rental['start_date']} to {rental['end_date']} ({rental['days']} days)")
            print(f"Cost: ${rental['total_cost']:.2f}")
            print(f"Status: {rental['status'].capitalize()}")
            if rental["status"] == "completed":
                print(f"Returned: {rental['return_date']}")
                if rental["fine_amount"] > 0:
                    print(f"Fine: ${rental['fine_amount']:.2f}")
    ...
    def view_profile(self):
        ...
        active = sum(1 for r in self.rentals if r["status"] == "active")
